...
Normal condition protein localization prediction file generation
'''
import argparse
import random
...
if __name__ == '__main__':
    ...
    parser.add_argument('-data', type=str, metavar='Dataset used for training\n'
                                                   '\t Available options: [GSE30931, GSE74572, GSE27182]\n'
                                                   '\t GSE30931 (Bortezomib)'
    ...
    log_path = '../data/log/' + dataflag + '/normal/'
    ...
    ecc = np.load('../data/generate_materials/ECC_normal_pca.npy')
    gcn = np.load(dataset + 'GCN_normal_pca.npy')
    loc = load_npz('../data/generate_materials/loc_matrix.npz')
    expr = np.load(dataset + 'expr_normal.npy')
