...
def create_graph(ppi, ecc, gcn, loc, expr, uniprot):
    ...
    :return:
        graph
    """
    start = list(ppi.row)  # edge start
    end = list(ppi.col)  # edge end
    num_nodes = len(uniprot)
    g = dgl.graph((start, end), num_nodes=num_nodes)
    g = dgl.add_self_loop(g)
    g.nodes[list(range(num_nodes))].data['loc'] = th.from_numpy(loc.toarray().astype(np.float32))  # loc feature
    edge_feat = np.hstack((gcn, ecc))  # concat gcn feature and ecc feature
    node_feat = th.tensor(np.hstack((expr, edge_feat)), dtype=th.float)  # concat expr and (gcn, ecc)
    g.nodes[list(range(num_nodes))].data['feat'] = node_feat  # construct feature vector
